...
# Caddy owns the public dashboard endpoints (:9860 and build.home).
# This UI/backend lives only on Raspi loopback and Caddy proxies to it.
BIND='127.0.0.1'
PORT=19861
    ...
    def do_GET(self):
        if self.path in ('/', '/index.html'):
            try:
                ...
                return self.send_bytes(500, str(e).encode(), 'text/plain; charset=utf-8')
            return self.send_bytes(200, body, 'text/html; charset=utf-8', 'no-store')

        if self.path == '/health':
            try:
                _, raw, _ = get_upstream('/health', 4)
                u = json.loads(raw)
                out = {
                    'ok': bool(u.get('ok')),
                    'owner': 'rapsi-nas',
                    'source': 'redmi-build',
                    'service': 'redmi-dashboard-ui',
                    'telemetry': u,
                }
        ...
        if self.path in ('/api/status', '/api/state', '/state'):
            try:
                ...
                out = {
                    ...
                    'error': str(e),
                    'runners': [], 'projects': [], 'runs': [], 'queue': [], 'history': [],
                }
                return self.send_bytes(503, json.dumps(out, separators=(',', ':')).encode(), 'application/json; charset=utf-8')
